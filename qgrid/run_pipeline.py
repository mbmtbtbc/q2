"""
Master pipeline: builds graph, encodes quantum weights, runs CTQW + classical
baseline + benchmark, runs density-matrix/noise analysis, simulates a cascade,
runs QAOA self-healing reroute, and dumps everything into data.json consumed
by the interactive dashboard (viz/dashboard.html).

Run:  python3 -m qgrid.run_pipeline
"""
import os
import json
import numpy as np
...
from qgrid.benchmark.metrics import benchmark_walks, total_variation_distance
...
def run(source="ieee33", seed_edge=(7, 8), source_bus=1, target_bus=18,
        gamma_deph=0.05, gamma_diff=0.01, partial_info_buses=(9, 15),
        topology_csv=None, load_csv=None):
    ...
    # ---------- 2. CTQW vs classical baseline + benchmark ----------
    ...
    target_idx = qw.idx[target_bus]
    stat = cw.stationary_distribution()
    bench = benchmark_walks(probs_q, probs_c, times, qw.idx[source_bus], target_idx, stat, dist_from_source)
    ...
    # ---------- 3. density matrix + noise (decoherence = partial information) ----------
    H, nodes_h, idx_h = build_hamiltonian(G0)
    partial_idx = [idx_h[b] for b in partial_info_buses if b in idx_h]
    dw = DensityMatrixWalk(H, gamma_deph=gamma_deph, gamma_diff=gamma_diff,
                            partial_info_nodes=partial_idx)
    dtimes = np.linspace(0, 6, 13)
    ...
    for i, snap in enumerate(snaps):
        cascade_rounds.append({
            "round": i,
            "edges": [{"u": u, "v": v, "status": snap[u][v]["status"],
                       "loading_pct": snap[u][v].get("loading_pct", 0.0)} for u, v in snap.edges()],
        })

    # ---------- 6. CTQW-informed QAOA self-healing reroute ----------
    qw_post = ContinuousTimeQuantumWalk(Gf)
    occ = qw_post.occupation_at(slack, t=3.0)
    tie_candidates = [(u, v) for u, v, d in Gf.edges(data=True) if d["is_tie_switch"]]
    h_qubo, J_qubo, cands = build_qubo(Gf, tie_candidates, occ)
    decision, qaoa_info = solve_qaoa_reroute(h_qubo, J_qubo, p=2, shots=1024, maxiter=40)
    ...
    healing_payload = {
        ...
        "healed_edges": [{"u": u, "v": v, "status": G_healed[u][v]["status"],
                           "loading_pct": G_healed[u][v].get("loading_pct", 0.0)} for u, v in G_healed.edges()],
    }
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(viz_dir, "qgrid_data.json")
    
    payload_json = json.dumps(payload)
    with open(data_path, "w", encoding="utf-8") as f:
        f.write(payload_json)
    print(f"wrote qgrid_data.json, {len(payload_json)} bytes")
    
    template_path = os.path.join(viz_dir, "dashboard_template.html")
    output_html_path = os.path.join(viz_dir, "dashboard.html")
    
    if os.path.exists(template_path):
        with open(template_path, "r", encoding="utf-8") as f:
            html_content = f.read()
        
        html_content = html_content.replace("__DATA_JSON__", payload_json)
        
        with open(output_html_path, "w", encoding="utf-8") as f:
            f.write(html_content)
        print("generated dashboard.html with injected data")
    else:
        print("warning: dashboard_template.html not found, skipping dashboard generation.")
